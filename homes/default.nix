...
in {
  home-manager = mkIf env.useHomeManager {
    ...
    users = genAttrs (
                        (lib.filter 
                                    (_user:config.modules.system.users.usersSettings. "${_user}".useHomeManager) 
                                    (lib.attrNames config.modules.system.users.usersSettings)
                        )
                     ) 
            (_user:./${_user} + /home.nix);
    # users = genAttrs (lib.attrNames config.modules.system.users.usersSettings) (_user:./${_user} + /home.nix);
